...
const RegularPropertiesScreen = () => {
  ...
  const fetchProperties = useCallback(async () => {
    try {
      ...
      setRegularProperties(categories.regularProperties);
      setTotalPages(
        Math.ceil(categories.regularProperties.length / PROPERTIES_PER_PAGE)
      );
    } catch (error) {
  ...
  const filterProperties = useCallback(() => {
    let filtered = [...regularProperties];
    ...
    return filtered;
  }, [regularProperties, searchQuery, filterCriteria, totalPages, currentPage]);
  ...
  const getUniqueValues = useCallback(
    (key) => {
      return [...new Set(regularProperties.map((item) => item[key]))].filter(
        Boolean
      );
    },
    [regularProperties]
      ...
      <>
        <View style={styles.headerContainer}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.headerTitle}>Regular Properties</Text>
          </View>
        </View>
